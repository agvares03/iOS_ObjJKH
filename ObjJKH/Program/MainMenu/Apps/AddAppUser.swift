...
//            print(request.url)
    ...
    func choice() {
        if (responseString == "1") {
            ...
            DispatchQueue.main.async(execute: {
                ...
                // Запишем заявку в БД
                let db = DB()
                db.add_app(id: 1, number: self.responseString, text: self.text_App.text!, tema: self.text_App.text!, date: self.date_teck()!, adress: "", flat: "", phone: "", owner: self.name_account, is_close: 1, is_read: 1, is_answered: 1)
    ...
    override func viewDidLoad() {
        ...
        lsCell.detailTextLabel?.text = appLsString()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        else if segue.identifier == "selectLS" {
            let selectItemController = (segue.destination as! UINavigationController).viewControllers.first as! SelectItemController
            selectItemController.strings = appLSs
            selectItemController.selectedIndex = appLS
            selectItemController.selectHandler = { selectedIndex in
                self.appLS = selectedIndex
                self.lsCell.detailTextLabel?.text = self.appLsString()
            }
        }
    }
